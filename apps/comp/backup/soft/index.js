define(['commonutil','ompage','pluspop','/apps/comp/model/tip/index.js'],function(ajaxUtil, page, pluspop, tip){
    ...
    obj.addDialog = function(){
        obj.clearAddHtml();
        selfDialog = dialog({
            id: 'backup_soft_add_dialog',
            title: '配置文件备份新增',
            content: $('#backup_soft_add_dialog')[0],
            width: 750,
            height: 380,
            okValue: '保存',
            ok: obj.save,
            ...
            ajaxUtil.callrest(compWsg.backup_manager_load,function(data){
                if(data.retCode == '0'){
                    require(['./tpl/edit.tpl'],function(his){
                        $("#backup_soft_edit_model").html(his(data.data.entity));
                        var selfDialog = dialog({
                            id: 'edit_conf_backup_his_dialog',
                            title: '软件文件备份修改',
                            content: $('#backup_soft_edit_model')[0],
                            width: 1100,
                            height: 500,
                            okValue: '修改',
                            ok : function(){
            ...
            ajaxUtil.callrest(compWsg.backup_manager_his,function(data){
                if(data.retCode == '0'){
                    require(['./tpl/his.tpl'],function(his){
                        $("#backup_soft_his_model").html(his(data.data));
                        var selfDialog = dialog({
                            id: 'soft_backup_his_dialog',
                            title: '配置文件备份历史    ',
                            content: $('#backup_soft_his_model')[0],
                            width: 1100,
                            height: 500,
                            cancelValue: '关闭',
                            cancel: function() {}
    ...
    obj.backIpAndBackDirectory = function () {
        ajaxUtil.callrest(compWsg.backup_manager_params,function(data){
            if(data.retCode == '0'){
                $("#soft_backup_ip").val(data.data.entity.backupIp);
                $("#soft_backup_path").val(data.data.entity.backupPath);
            }else{
                pluspop.alert(data.retMsg);
            }
        },null,true);
